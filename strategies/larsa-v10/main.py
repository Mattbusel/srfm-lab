# LARSA v10: v9 + volatility-targeted position sizing
#
# Core change from v9: Replace fixed TF_CAP fraction with vol-targeted sizing.
#
# WHY:
#   v9 uses fixed fractions (e.g. TF_CAP[7]=0.65).
#   At $15M equity with vol=0.3%/hr → position = 0.65 × $15M = $9.75M notional
#   At Volmageddon (vol=1.8%/hr) → SAME $9.75M notional → 3× the daily dollar risk
#   → margin cascade becomes self-reinforcing
#
# THE FIX: size positions to target a FIXED daily dollar risk fraction of equity.
#   daily_vol_pct = atr_hourly_pct × sqrt(6.5)   # annualize to daily
#   raw_size      = TARGET_DAILY_RISK / daily_vol_pct
#   tgt           = min(raw_size, TF_CAP[tf_score]) × direction
#
# WHAT THIS DOES:
#   Normal vol  (Jan 2018, ATR=0.3%/hr):  raw_size = 0.01/0.0077 = 1.30 → capped at TF_CAP
#   Volmageddon (Feb 2018, ATR=1.8%/hr):  raw_size = 0.01/0.0459 = 0.22 → automatic 3× deleverage
#   Works at $1M or $100M — always targeting same RISK fraction, not same NOTIONAL fraction.
#
# All other v9 logic (4-bar hold, hourly gate, portfolio cap, regime detection) preserved intact.
#
# QC NOTE: Resolution.FIFTEEN_MINUTE does not exist in QC.
#          15min instruments subscribe at Resolution.Minute and consolidate via timedelta.
...
# tf_score → position CEILING (vol-targeting may produce lower values)
TF_CAP = {7: 0.65, 6: 0.55, 5: 0.45, 4: 0.35, 3: 0.30, 2: 0.25, 1: 0.15, 0: 0.0}

# v10: target 1% of equity as daily dollar risk per instrument
# At normal vol this hits TF_CAP ceiling. At vol spikes it auto-deleverages.
...
class LarsaV10(QCAlgorithm):
    ...
    def on_data(self, data):
        ...
        # ── Step 1: vol-targeted raw targets ────────────────────────────
        raw_targets = {}
        ...
        for sym in ["ES", "NQ", "YM"]:
            ...
            if ceiling == 0.0:
                ...
                if direction == 0:
                    ...
                    # ── v10: volatility-targeted sizing ─────────────────
                    # Scale position so expected daily dollar loss = TARGET_DAILY_RISK × equity.
                    # daily_vol_pct = hourly ATR% × sqrt(6.5 trading hours/day)
                    # raw_size = TARGET_DAILY_RISK / daily_vol_pct
                    # cap at TF_CAP ceiling so we never over-size in calm markets either.
                    if i1h.atr.is_ready and i1h.atr.current.value > 0:
                        ...
                        if price > 0:
                            ...
                            raw_size = TARGET_DAILY_RISK / (daily_vol_pct + 1e-9)
                            cap = min(raw_size, ceiling)
                        else:
                            cap = ceiling
                    else:
                        cap = ceiling   # fallback to fixed cap if ATR not ready
                    # ─────────────────────────────────────────────────────
        ...
        if any(i.bc % 24 == 0 for i in self.instr_1h.values()):
            ...
            # Plot actual vol-targeted sizes for each instrument
            for sym in ["ES", "NQ", "YM"]:
                i1h = self.instr_1h[sym]
                mapped = i1h.future.mapped
                if mapped and i1h.atr.is_ready and i1h.atr.current.value > 0:
                    try:
                        ...
                        if price > 0:
                            hourly_vol_pct = i1h.atr.current.value / price
                            daily_vol_pct  = hourly_vol_pct * math.sqrt(6.5)
                            vol_size = min(TARGET_DAILY_RISK / (daily_vol_pct + 1e-9), 0.65)
                            self.plot("VolSizing", f"{sym}_size", vol_size)
                    except Exception:
                        pass
    ...
    def on_end_of_algorithm(self):
        ...
        for sym in ["ES", "NQ", "YM"]:
            ...
            i1h = self.instr_1h[sym]
            i1d = self.instr_1d[sym]
            # Log final vol sizing for context
            ...
            if mapped and i1h.atr.is_ready and i1h.atr.current.value > 0:
                try:
                    ...
                    if price > 0:
                        hourly_vol_pct = i1h.atr.current.value / price
                        daily_vol_pct  = hourly_vol_pct * math.sqrt(6.5)
                        vol_size = TARGET_DAILY_RISK / (daily_vol_pct + 1e-9)
                        vol_info = f"atr={i1h.atr.current.value:.1f} daily_vol={daily_vol_pct:.4f} vol_size={vol_size:.3f}"
                except Exception:
                    pass
            self.log(
                f"[{sym}] END "
