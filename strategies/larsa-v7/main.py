# LARSA v7: convergence-first sizing | solo cap 0.20 | pos_floor conv-only | NO SIDEWAYS GATE
# v6 post-mortem: SIDEWAYS ctl>=8 gate blocked first legs of convergence events (124→377 trades needed)
# v7 fix: remove SIDEWAYS gate entirely. Size differentially but don't gate entries.
# region imports
...
class LarsaMultiFuturesAlgorithm(QCAlgorithm):
    ...
    def on_data(self, data):
        ...
        for key, inst in self.instruments.items():
            ...
            # v7: convergence-first sizing — no entry gating (v6 lesson: extra gates killed conv legs)
            # Solo BH: 50% WR per forensics → small cap, but ALLOWED (may be first leg of convergence)
            # Convergence (2+ BH active): 74.5% WR, 24.5x more profitable → full cap
            # Kelly: conv half-Kelly=27.9%, solo half-Kelly=1.9% → ratio ~14x
            if bh_active_count >= 2:
                cap = 0.65   # full convergence size
            else:
                cap = 0.20   # solo: small but allowed — may become first leg of convergence
    ...
    def _process_instrument(self, data, inst, rm, weight):
        ...
        # v7: SIDEWAYS gate REMOVED — it blocked first legs of convergence events (v6 post-mortem)
        # Solo in SIDEWAYS is near break-even but must be allowed: it may become convergence leg 1.
