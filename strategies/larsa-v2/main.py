# region imports
# LARSA v2: BEAR_FAST tl_req=2 | ATR_SCALE compound protection | STOP_LOSS -3% well cut | CONV_MASS smoother
...
class LarsaMultiFuturesAlgorithm(QCAlgorithm):
    ...
    def _process_instrument(self, data, inst, rm, weight):
        ...
        tl_req = 1 if inst.regime == MarketRegime.HIGH_VOLATILITY else (2 if inst.regime == MarketRegime.BEAR else 3)  # v2 BEAR_FAST
        ...
        # v2 STOP_LOSS: exit if well P&L < -3% of portfolio at well entry
        if inst.bh_active and inst.well_entry_pv > 0 and not killed:
            pv_chk = self.portfolio.total_portfolio_value
            if (pv_chk - inst.well_entry_pv) / (inst.well_entry_pv + 1e-9) < -0.03:
                tgt = 0.0; killed = True; inst.pos_floor = 0.0; inst.well_entry_pv = 0.0
