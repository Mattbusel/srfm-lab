# LARSA v4: NQ notional cap $400k | BEAR long gate rhb>5 | + v3 CONV_SIZE + pos_floor
# region imports
...
class LarsaMultiFuturesAlgorithm(QCAlgorithm):
    ...
    def on_data(self, data):
        ...
        for key, inst in self.instruments.items():
            ...
            # v4: NQ notional cap — NQ $20/pt makes leverage asymmetric vs ES/YM
            if key == "NQ" and inst.mapped_sym is not None:
                nq_price = inst.cw[0] if inst.cw.count > 0 else 15000.0
                nq_dollar_notional = abs(tgt) * self.portfolio.total_portfolio_value
                # Cap NQ at $400k notional (reduces from ~$2M at peak to manageable)
    ...
    def _process_instrument(self, data, inst, rm, weight):
        ...
        # v4: BEAR regime long gate — block longs in sustained BEAR
        # BEAR is 96.6% persistent (avg 29.5 bars). Longs in sustained BEAR = low edge.
        # Only allow longs if regime recently changed (rhb <= 5 bars in BEAR = transition entry)
        if inst.regime == MarketRegime.BEAR and tgt > 0 and inst.rhb > 5:
            tgt = 0.0; killed = True  # v4: no longs in sustained BEAR
