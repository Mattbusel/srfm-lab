"""Expand inference.py with large additions."""
import os

INFERENCE_PATH = os.path.join(os.path.dirname(__file__), "..", "lumina", "inference.py")
...
with open(INFERENCE_PATH, "a", encoding="utf-8") as f:
    f.write(CONTENT)

import subprocess, sys
result = subprocess.run([sys.executable, "-c",
    "import subprocess; lines = open(r'" + INFERENCE_PATH.replace('\\', '/') + "').readlines(); print(len(lines))"],
    capture_output=True, text=True)
print(result.stdout.strip(), INFERENCE_PATH)
